...
static Vec3 QuaternionToEuler(float _x, float _y, float _z, float _w, bool _retDegree)
{
	XMFLOAT4 quaternion = XMFLOAT4(_x, _y, _z, _w);
...
static Vec3 QuaternionToEuler(float _x, float _y, float _z, float _w)
{
	float angle;
	XMVECTOR axis;
	XMQuaternionToAxisAngle(&axis, &angle, Quaternion(_x, _y, _z, _w));

	Vec3 euler;

	// Roll (around Z-axis)
	euler.z = atan2f(XMVectorGetY(axis), XMVectorGetX(axis));

	// Pitch (around X-axis)
	float sinP = 2.0f * (XMVectorGetW(axis) * XMVectorGetY(axis) - XMVectorGetZ(axis) * XMVectorGetX(axis));
	if (fabsf(sinP) >= 1.0f)
		euler.x = copysignf(XM_PI / 2.0f, sinP); // Use 90 degrees if out of range
	else
		euler.x = asinf(sinP);

	// Yaw (around Y-axis)
	float sinY = 2.0f * (XMVectorGetW(axis) * XMVectorGetZ(axis) - XMVectorGetX(axis) * XMVectorGetY(axis));
	float cosY = 1.0f - 2.0f * (XMVectorGetY(axis) * XMVectorGetY(axis) + XMVectorGetZ(axis) * XMVectorGetZ(axis));
	euler.y = atan2f(sinY, cosY);

	return Vec3(euler.x * XM_PI / 180.f, euler.y * XM_PI / 180.f, euler.z * XM_PI / 180.f);
}

static float Truncated(float _value)
{
	return std::floor(_value * 10) / 10;
}
