#ifndef _LORDBOSS_DEFERRED_FX_
#define _LORDBOSS_DEFERRED_FX_
...
PS_OUT PS_Main(VS_OUT _in)
{
    ...
    if (length(g_bloomFilter) > 0)
    {
        ...
        if (output.color.r >= max(output.color.g, output.color.b) + 0.1f)
            output.bloom = float4(1.f, 0.f, 0.f, 1.f);
        else if (output.color.g >= max(output.color.r, output.color.b) + 0.1f)
            output.bloom = float4(0.f, 1.f, 0.f, 1.f);
        else if (output.color.b >= max(output.color.g, output.color.r) + 0.1f)
            output.bloom = float4(1.f, 0.f, 0.f, 1.f);
        else
